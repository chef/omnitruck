...
context 'Omnitruck' do
  ...
  context "download / metadata endpoints" do
    ...
    context "for chef" do
      ...
      context 'for stable' do
        ...
        context 'for amazon linux' do
          ...
          context 'for 2' do
            # For chef-client, Amazon Linux 2 should always map to EL7
            let(:expected_platform) { 'el' }
    ...
    context 'for chef-server-core' do
      let(:project) { "chef-server-core" }
        ...
        context 'for amazon' do
          ...
          context 'for 2' do
            ...
            context 'for x86_64' do
              ...
              context 'with version >= 15.10.12' do
                let(:project_version) { '15.10.12' }
                # For chef-server-core versions >= 15.10.12, keep as amazon
                let(:expected_platform) { 'amazon' }
                let(:expected_platform_version) { '2' }

                it_behaves_like 'a correct package info'
              end

              context 'with version < 15.10.12' do
                let(:project_version) { '15.10.11' }
                # For chef-server-core versions < 15.10.12, map to el
                let(:expected_platform) { 'el' }
                let(:expected_platform_version) { '7' }
